...
| — | **build-tools plugin ≥ 1.0.137** (ZB_API_KEY split + org-task @Internal fix — hard floor for org loads, see "Hard version floors") | `./gradlew buildEnvironment \| grep build-tools` from the repo root → version ≥ 1.0.137 | below floor with `~/.m2/repository/com/zerobias/build-tools` present → delete that dir and re-check (a local build shadows the release); otherwise fix the GitHub-token row above (the plugin resolves from GH Packages) |
